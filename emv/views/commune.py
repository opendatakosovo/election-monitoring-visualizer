...
class Commune(View):	
	...
	def dispatch_request(self, observer, year, election_type, election_round, commune):
		...
		kvv_request_url ='%s/kvv-members-gender-distribution/%d/%s/%s/%s' % (kdi_api_url, year, election_type, election_round, commune)
		...
		# Request the JSON Document from the Election-Monitoring-API (shortcut EMA) based on commune
		votes_by_hours_request_url ='%s/votes-count/%d/%s/%s/%s' % (kdi_api_url, year, election_type, election_round, commune)
