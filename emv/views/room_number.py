...
class RoomNumber(View):
	...
	def dispatch_request(self, observer, year, election_type, election_round, commune, polling_station_name, room_number):
		
		# get KDI api url
		kdi_api_url = utils.get_api_url(observer)
	
		#URL to request from KDI API for KVV MEMBERS GENDER DISTRIBUTION
		url = '%s/kvv-members-gender-distribution/%d/%s/%s/%s/%s/%s' % (kdi_api_url, year, election_type, election_round, commune, polling_station_name, room_number)
		...
		url1 = '%s/votes-count/%d/%s/%s/%s/%s/%s' % (kdi_api_url, year, election_type, election_round, commune, polling_station_name,room_number)
