...
class Search(MethodView):	
	...
	def dispatch_request(self, observer, year, election_type, election_round):
		...
		url_params = (api_url, year, election_type, election_round, cmn, ps, uvc, fs)

		search_url = '%s/search/%d/%s/%s/?commune_select=%s&polling_station_select=%s&ultra_violet_control_select=%s&finger_sprayed_select=%s' % url_params
